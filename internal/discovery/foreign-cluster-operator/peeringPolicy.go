...
func (r *ForeignClusterReconciler) getDesiredOutgoingPeeringState(ctx context.Context,
	...
	peeringEnabled, err := r.isPeeringEnabled(ctx, foreignCluster)
	if err != nil {
		klog.Error(err)
	...
	if remoteNamespace != "" && peeringEnabled {
		return desiredPeeringPhasePeering
	}
	return desiredPeeringPhaseUnpeering
}

func (r *ForeignClusterReconciler) isPeeringEnabled(ctx context.Context,
	foreignCluster *discoveryv1alpha1.ForeignCluster) (bool, error) {
	switch foreignCluster.Spec.PeeringEnabled {
